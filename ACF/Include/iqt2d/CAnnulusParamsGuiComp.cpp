...
void CAnnulusParamsGuiComp::OnGuiModelAttached()
{
	...
	if (m_unitNameAttrPtr.IsValid()){
		...
		InnerRadiusUnitLabel->setText(iqt::GetQString(*m_unitNameAttrPtr));
		OuterRadiusUnitLabel->setText(iqt::GetQString(*m_unitNameAttrPtr));
	}
	else{
		PositionUnitLabel->setVisible(false);
		InnerRadiusUnitLabel->setVisible(false);
		OuterRadiusUnitLabel->setVisible(false);
	}
}


void CAnnulusParamsGuiComp::OnGuiModelDetached()
{
