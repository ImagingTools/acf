...
function getSimplified(list)
{
    ...
    for (i in list){
        ...
        if (usedMap[element] !== true){
            retVal.unshift(element);

            usedMap[element] = true;
        }
    }
...
function libraryLinkerFlags(product, inputs)
{
    ...
    var staticLibraries = getSimplified(ModUtils.modulePropertiesFromArtifacts(product, inputs.staticlibrary, 'cpp', 'staticLibraries'));
    ...
    if (libraryPaths)
        args = args.concat(getSimplified(libraryPaths.map(function(path) { return '-L' + path })));
    if (frameworkPaths)
        args = args.concat(getSimplified(frameworkPaths.map(function(path) { return '-F' + path })));
    if (systemFrameworkPaths)
        args = args.concat(getSimplified(systemFrameworkPaths.map(function(path) { return '-iframework' + path })));
