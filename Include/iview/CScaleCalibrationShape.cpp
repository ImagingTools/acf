#include "iview/CScaleCalibrationShape.h"
...
void CScaleCalibrationShape::Draw(QPainter& drawContext) const
{
	const CScaleCalibration* calibPtr = dynamic_cast<const CScaleCalibration*>(GetCalibrationPtr());
	if (calibPtr == NULL){
	...
	if (IsDisplayConnected()){
		...
		if (calibInfoPtr != NULL){
			...
			if (isGridVisible || leftRulerPtr.IsValid() || topRulerPtr.IsValid()){
				...
				i2d::CAffine2d calibTransform = calibPtr->GetLogToViewTransform();
